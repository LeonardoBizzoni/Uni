parent(Parent, Child) :- father(Parent, Child), male(Parent).
parent(Parent, Child) :- mother(Parent, Child), female(Parent).

sibling(X, Y) :- parent(Parent, X),
		 parent(Parent, Y),
		 X \= Y.

brother(Brother, Y) :- sibling(Brother, Y),
		       male(Brother).

sister(Sister, Y) :- sibling(Sister, Y),
		     female(Sister).

uncle(Uncle, X) :- brother(Uncle, Parent), parent(Parent, X).
aunt(Aunt, X) :- sister(Aunt, Parent), parent(Parent, X).

decendant(Decendant, Ancestor) :- parent(Ancestor, Decendant).
decendant(Decendant, Ancestor) :- parent(Parent, Decendant),
				  decendant(Parent, Ancestor).

ancestor(Ancestor, Decendant) :- descendant(Decendant, Ancestor).
